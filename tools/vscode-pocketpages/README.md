...
- `_private/*.ejs` partial 파일에서 include 호출부 역참조 찾기
- `_private/*.js` private module 파일에서 resolve 호출부 역참조 찾기
- static route 파일에서 `href`, `action`, `hx-*`, `redirect()` 사용처 역참조 찾기
...
- `_private/*.ejs` partial을 누가 include하는지 역으로 찾을 때
- `_private/*.js` service/role/module을 어디서 resolve하는지 찾을 때
- page/xapi/api 엔트리를 어디서 route string으로 가리키는지 찾을 때
