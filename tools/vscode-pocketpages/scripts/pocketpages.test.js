'use strict'
const { test: registerTest } = require('node:test')

// Preserve the original scopes and sequential execution during this migration.
// Fixture strings and verification bodies intentionally retain their original formatting.
// BEGIN original lsp-regression.test.js
{
"use strict";
...
test("snapshots compare arbitrary generations without retaining their history", async () => {
  ...
  const originals = ["abc", "abXc", "", "한글😀\r\nconst value = 1;", "한글😀\r\nconst value = 12;"];
  const snapshots = originals.map((text) => createScriptSnapshot(text));
  for (const [oldIndex, oldSnapshot] of snapshots.entries()) {
    for (const [newIndex, snapshot] of snapshots.entries()) {
      const change = snapshot.getChangeRange(oldSnapshot);
      const { start, length } = change.span;
      assert.equal(
        originals[oldIndex].slice(0, start) + snapshot.getText(start, start + change.newLength) +
          originals[oldIndex].slice(start + length),
        originals[newIndex]
      );
    }
  }
  assert.deepEqual(snapshots[1].getChangeRange(snapshots[0]), {
    span: { start: 2, length: 0 }, newLength: 1,
  });
  assert.equal(snapshots[0].getChangeRange(undefined), undefined);

  let latest = null;
  const history = [];
  for (let version = 0; version < 512; version += 1) {
    // Pass the previous snapshot to also exercise callers using the old optional argument.
    latest = createScriptSnapshot(`${version}:` + "x".repeat(8000), latest);
    history.push(new WeakRef(latest));
  }
  for (let attempt = 0; attempt < 3; attempt += 1) {
    await nextTurn();
    global.gc();
  }
  assert.equal(history.filter((entry) => entry.deref()).length, 1,
    "Only the current snapshot should survive GC");
  assert.equal(latest.getText(0, 4), "511:");
});

test("TS mirror preserves UTF-16 offsets, newlines and server code", () => {
  for (const newline of ["\n", "\r\n"]) {
    for (const html of ["plain", "한글", "😀", "👩‍💻📚"]) {
      const text = [
        `<p>${html}</p>`,
        "<script server>",
        "const title = '한글😀';",
        ...
        "</script>",
        `<p>${html}</p>`,
        "<script server>const count = 1;</script>",
        "",
      ].join(newline);
      const mirrored = buildScriptServerMirrorText(text);
      assert.equal(mirrored.length, text.length, `UTF-16 length: ${html}`);
      const blocks = extractServerBlocks(text);
      for (let offset = 0; offset < text.length; offset += 1) {
        const isCode = blocks.some((block) => offset >= block.contentStart && offset < block.contentEnd);
        const expected = isCode || text[offset] === "\n" || text[offset] === "\r" ? text[offset] : " ";
        assert.equal(mirrored[offset], expected, `code unit at ${offset}`);
      }
    }
  }
});

test("near completion reuse requires the same service and project version", () => {
  const service = {};
  const request = { uri: "file:///page.ejs", version: 1, line: 1, character: 10,
    triggerKind: 3, service, projectVersion: 5 };
  const cached = { ...request, result: { isIncomplete: true, items: [] } };
  assert.equal(shouldReuseLastCompletion(cached, request), true);
  assert.equal(shouldReuseLastCompletion(cached, { ...request, projectVersion: 6 }), false);
...
function createApp(fixtureRoot, name, collection) {
  const appRoot = path.join(fixtureRoot, "apps", name);
  const pagesRoot = path.join(appRoot, "pb_hooks", "pages");
  fs.mkdirSync(path.join(pagesRoot, "_private"), { recursive: true });
  writeFile(path.join(appRoot, "pb_data", "types.d.ts"), `
declare namespace core {
  interface Record { id: string; get(name: string): any; }
  interface App { findRecordById(collectionModelOrIdentifier: any, id: string): core.Record; }
}
declare namespace pocketbase { interface PocketBase extends core.App {} }
declare var $app: pocketbase.PocketBase;
`);
  writeFile(path.join(appRoot, "pocketpages-globals.d.ts"), `
declare const include: (path: string, locals?: Record<string, any>) => string;
declare const resolve: (path: string) => any;
`);
  writeFile(path.join(appRoot, "types.d.ts"), "");
  const schemaPath = path.join(appRoot, "pb_schema.json");
  const schemaText = JSON.stringify([{ name: collection, fields: [{ name: "title", type: "text" }] }]);
  writeFile(schemaPath, schemaText);
  return { appRoot, pagesRoot, schemaPath, schemaText };
}
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error(`IPC timeout after ${milliseconds}ms`)), milliseconds);
      }),
    ]);
  } finally {
    clearTimeout(timer);
  ...
  t.after(async () => {
    try {
      if (child.exitCode === null && child.signalCode === null) {
        await withDeadline(connection.sendRequest("shutdown"), 2000);
        await notify("exit");
        await withDeadline(exited, 2000);
      }
    } catch (error) {
      t.diagnostic(`Server cleanup: ${error.message}\n${stderr}`);
    } finally {
      connection.dispose();
      if (child.exitCode === null && child.signalCode === null) {
        child.kill();
      ...
      assert.ok(path.basename(target).startsWith("pocketpages-lsp-regression-"));
      fs.rmSync(target, { recursive: true, force: true });
    }
  });

  await request("initialize", { processId: process.pid, rootUri: uri(fixtureRoot),
    capabilities: { workspace: { diagnostics: { refreshSupport: true } } } });
  await notify("initialized", {});

  return {
    fixtureRoot, logs, request, notify, uri,
    async open(filePath, text) {
      await notify("textDocument/didOpen", { textDocument: { uri: uri(filePath), languageId: "ejs", version: 1, text } });
    },
    async change(filePath, version, text) {
      await notify("textDocument/didChange", { textDocument: { uri: uri(filePath), version }, contentChanges: [{ text }] });
    },
    ...
    async watch(filePath, type = 2) {
      await notify("workspace/didChangeWatchedFiles", { changes: [{ uri: uri(filePath), type }] });
    },
  };
}
...
test("IPC completion follows another open partial and preserves app isolation", { timeout: 30000 }, async (t) => {
  ...
  const app = createApp(server.fixtureRoot, "first", "posts");
  const other = createApp(server.fixtureRoot, "second", "journals");
  const page = path.join(app.pagesRoot, "caller.ejs");
  const partial = path.join(app.pagesRoot, "_private", "card.ejs");
  const otherPage = path.join(other.pagesRoot, "index.ejs");
  const callerText = "<%- include('card.ejs', {  }) %>";
  const otherText = "<script server>$app.findRecordById('', 'id')</script>";
  writeFile(page, callerText);
  writeFile(partial, "<%= beforeName %>");
  writeFile(otherPage, otherText);
  await server.open(page, callerText);
  await server.open(partial, "<%= beforeName %>");
  await server.open(otherPage, otherText);
  const offset = callerText.indexOf("{  ") + 2;
  const otherOffset = otherText.indexOf("('',") + 2;
  assert.deepEqual(await server.complete(page, callerText, offset), ["beforeName"]);
  assert.deepEqual(await server.complete(otherPage, otherText, otherOffset), ["journals"]);

  await server.change(partial, 2, "<%= afterName %>");
  assert.deepEqual(await server.complete(page, callerText, offset), ["afterName"]);
  let logStart = server.logs.length;
  assert.deepEqual(await server.complete(page, callerText, offset), ["afterName"]);
  assert.ok(server.logs.slice(logStart).some((line) => line.includes("case=exact-cache")), "Stable requests still use the exact cache");
  logStart = server.logs.length;
  assert.deepEqual(await server.complete(otherPage, otherText, otherOffset), ["journals"]);
  assert.ok(server.logs.slice(logStart).some((line) => line.includes("case=exact-cache")), "Other app caches remain usable");

  // Closing an unsaved partial returns its contract to the disk version.
  await server.notify("textDocument/didClose", { textDocument: { uri: server.uri(partial) } });
  assert.deepEqual(await server.complete(page, callerText, offset), ["beforeName"]);
});
...
test("IPC schema watch retains valid data only through parse failures", { timeout: 30000 }, async (t) => {
  ...
  const text = "<script server>$app.findRecordById('', 'id')</script>";
  writeFile(page, text);
  await server.open(page, text);
  const complete = () => server.complete(page, text, text.indexOf("('',") + 2);
  const updateSchema = async (value) => { writeFile(app.schemaPath, value); await server.watch(app.schemaPath); };
  assert.deepEqual(await complete(), ["posts"]);
  await updateSchema("[");
  assert.deepEqual(await complete(), ["posts"]);
  await updateSchema('{"unfinished":');
  assert.deepEqual(await complete(), ["posts"]);
  await updateSchema(JSON.stringify([{ name: "books", fields: [] }]));
  assert.deepEqual(await complete(), ["books"]);
  await updateSchema("[]");
  assert.deepEqual(await complete(), []);
  await updateSchema("[");
  assert.deepEqual(await complete(), [], "A valid empty schema replaces the previous schema");
  await updateSchema(app.schemaText);
  assert.deepEqual(await complete(), ["posts"]);
  fs.unlinkSync(app.schemaPath);
  await server.watch(app.schemaPath, 3);
  assert.deepEqual(await complete(), [], "Deletion clears the last valid schema");
  writeFile(app.schemaPath, "[");
  await server.watch(app.schemaPath, 1);
...
// END original lsp-regression.test.js

// BEGIN original extension-host-sanity.js
const runExtensionHostSanityCheck = (() => {
'use strict'
...
return runExtensionHostSanityCheck
})()
// END original extension-host-sanity.js

// BEGIN original sanity-check.js
{
'use strict'
...
registerTest('PocketPages full sanity (including extension host)', { concurrency: false }, run)
}
// END original sanity-check.js
