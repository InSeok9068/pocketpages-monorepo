...
## 순수 JS 라이브러리 후보 (백엔드) - PocketBase JSVM
...
| `lodash`           | 배열/객체/문자열 처리를 위한 범용 JS 유틸리티   |
| `validator`        | 이메일, URL, 숫자/문자열 같은 문자열 단위 검증  |
| `zod`              | form/body/query 입력값 런타임 검증              |
| `dompurify`        | 외부·사용자 HTML의 XSS 정제                     |
| `marked`           | Markdown을 HTML로 변환                          |
...
| `@floating-ui/dom` | 팝오버, 메뉴, 툴팁 같은 요소의 위치 계산        |
| `tom-select`       | 검색, 다중 선택, 태그 입력이 가능한 select UI   |
| `filepond`         | 파일 업로드, 미리보기, 확장자·크기 검증         |
| `tabulator-tables` | 데이터 테이블 정렬, 필터, 페이지 이동, 셀 편집  |
