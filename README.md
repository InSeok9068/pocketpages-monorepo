...
- `_private/*.ejs` partial 기준 `PocketPages: Find Current Partial References` 명령 지원
- `_private/*.js` resolve 역참조, static route file route 역참조 명령 지원
- `href`, `action`, `hx-*`, `redirect()`의 정적 라우트 문자열 자동완성 및 정의 이동
- `resolve()`로 불러온 CommonJS 모듈 export 멤버에 대한 definition, references, rename 지원
- `_private`/route 파일 상단 CodeLens와 에디터 우클릭 reference 명령 지원
