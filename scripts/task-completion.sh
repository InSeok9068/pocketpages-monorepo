...
_pp_dev_complete() {
  ...
  if [[ $COMP_CWORD -eq 1 ]]; then
    COMPREPLY=( $(compgen -W "start kill update audit install deploy rollback backup archive restore archives merge test lint tsc diag verify preflight knip gitleaks index new css bundle generate format help" -- "$cur") )
    return
  fi
