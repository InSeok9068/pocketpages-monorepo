...
_pp_dev_complete() {
  ...
  if [[ $COMP_CWORD -eq 1 ]]; then
    COMPREPLY=( $(compgen -W "start kill update audit install deploy rollback docker-rollback backup archive restore archives merge test lint tsc diag verify preflight knip gitleaks index new css bundle generate format help" -- "$cur") )
    return
  fi
  ...
  if [[ $COMP_CWORD -eq 2 && ( "$cmd" == "start" || "$cmd" == "deploy" || "$cmd" == "rollback" || "$cmd" == "docker-rollback" || "$cmd" == "backup" || "$cmd" == "archive" || "$cmd" == "test" || "$cmd" == "lint" || "$cmd" == "tsc" || "$cmd" == "diag" || "$cmd" == "verify" || "$cmd" == "index" || "$cmd" == "css" ) ]]; then
  ...
  if [[ $COMP_CWORD -eq 3 && "$cmd" == "docker-rollback" ]]; then
    COMPREPLY=( $(compgen -W "1 2 3" -- "$cur") )
    return
  fi
  ...
  if [[ "$cmd" == "audit" && "$cur" == --* ]]; then
    COMPREPLY=( $(compgen -W "--omit=dev --audit-level --json --help" -- "$cur") )
    return
  fi
  ...
  if [[ "$cmd" == "new" ]]; then
    if [[ $COMP_CWORD -ge 3 && "${COMP_WORDS[COMP_CWORD-1]}" == "--features" ]]; then
      COMPREPLY=( $(compgen -W "htmx,alpine htmx alpine datastar realtime none" -- "$cur") )
      return
    fi
