...
function printServiceResult(result) {
  ...
  for (const issue of result.issues) {
    ...
    console.log(
      `  ${displayPath}:${issue.line}:${issue.column} [${toSeverityLabel(issue.category)}][${String(issue.code)}] ${issue.message}`
    )
  }

  console.log(
    `Found ${result.issues.length} issue(s) in ${(result.totalMs / 1000).toFixed(2)}s.`
  )
}
