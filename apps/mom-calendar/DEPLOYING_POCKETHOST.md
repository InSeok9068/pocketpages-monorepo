...
- `PHIO_DEPLOY_KEY_B64`: 로컬 `phio_deploy_ed25519` private key를 base64로 인코딩한 값
...
`PHIO_DEPLOY_KEY_B64`는 PocketHost Settings > Keys에 등록된 `Phio` public key와 짝이 맞는 private key를 base64로 인코딩한 값입니다. Windows 기본 위치는 보통 아래 경로입니다.

```powershell
[Convert]::ToBase64String([IO.File]::ReadAllBytes("$env:APPDATA\phio-nodejs\Config\phio_deploy_ed25519")) | Set-Clipboard
```
