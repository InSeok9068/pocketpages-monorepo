;(function () {
  ...
  function updatePermissionButtons() {
    ...
    for (let index = 0; index < descriptions.length; index += 1) {
      ...
      if (permission === 'granted') description.textContent = '예약한 일정이 되면 브라우저 알림으로 알려드립니다.'
        ...
        description.textContent = '내부망 주소에도 HTTPS를 적용해야 브라우저 알림을 사용할 수 있습니다.'
      else if (permission === 'unsupported')
        description.textContent = '현재 브라우저에서는 시스템 알림을 사용할 수 없습니다.'
  ...
  Promise.resolve(
    previousRealtime && typeof previousRealtime.stop === 'function' ? previousRealtime.stop() : null
  ).finally(function () {
    resumeRealtime()
  })
})()
