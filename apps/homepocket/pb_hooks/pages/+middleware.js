...
function renderErrorPage(title, message, detail) {
  const detailHtml = detail
    ? '<pre style="margin-top:16px;white-space:pre-wrap;border-radius:8px;background:#fff7ed;padding:16px;color:#7c2d12;">'
      + escapeHtml(detail)
      + '</pre>'
    : ''
  ...
  return (
    ...
    + '<p style="margin:0 0 10px;font-size:12px;font-weight:700;letter-spacing:.12em;color:#64748b;text-transform:uppercase;">homepocket</p>'
...
module.exports = function (api, next) {
  ...
  try {
    ...
    error('homepocket/global-error-boundary:caught', {
      pathname: pathname,
    ...
    if (isXapiPath(pathname)) {
      ...
      dbg('homepocket/global-error-boundary:redirect', {
        status: 303,
        redirectTo: '/',
