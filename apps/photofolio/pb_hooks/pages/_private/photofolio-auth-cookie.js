 ...
 * @param {number} fallback 기본 초 값입니다.
...
function readAuthCookieConfig(envGetter) {
  ...
  return {
    maxAgeSeconds: readPositiveIntegerEnv(envGetter, 'PHOTOFOLIO_AUTH_COOKIE_MAX_AGE_SECONDS', DEFAULT_AUTH_COOKIE_MAX_AGE_SECONDS),
 ...
 * @param {any} record PocketBase auth record입니다.
 * @returns {any} JSON 직렬화 가능한 값입니다.
 */
 ...
 * @param {(key: string) => string} envGetter 환경 변수 조회 함수입니다.
 * @param {string} email 로그인 이메일입니다.
 * @param {string} password 로그인 비밀번호입니다.
 * @returns {{ token: string, record: any }} 인증 결과입니다.
 */
...
/**
 * 응답에 로그인 쿠키를 씁니다.
 * @param {import('pocketpages').PagesResponse} response 현재 응답 객체입니다.
 * @param {{ token: string, record: any }} authData 인증 결과입니다.
 * @param {(key: string) => string} envGetter 환경 변수 조회 함수입니다.
 */
