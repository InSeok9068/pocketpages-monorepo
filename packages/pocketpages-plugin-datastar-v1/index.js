...
function buildNavigationScript(options) {
  ...
  const navigationStateKey = '__pocketpagesDatastarNavigation'
  const clickExpression = [
    ...
    'var navigation=window.' + navigationStateKey,
    'if(navigation.controller)navigation.controller.abort()',
    'navigation.controller=new AbortController()',
    'history.pushState({datastar:{url:path}},"",path)',
    '@get(path,{headers:' + headersJson + ',requestCancellation:navigation.controller})',
  ].join(';')
  const popstateExpression = [
    ...
    'if(navigation.controller)navigation.controller.abort()',
    'navigation.controller=new AbortController()',
    '@get(url,{headers:' + headersJson + ',requestCancellation:navigation.controller})',
  ].join(';')

  return [
    '<script>',
    '(function () {',
    '  var navigationStateKey = ' + scriptJson(navigationStateKey) + ';',
    '  var navigationState = window[navigationStateKey];',
    '  if (!navigationState) {',
    '    navigationState = { controller: null };',
    '    window[navigationStateKey] = navigationState;',
    '    window.addEventListener("pagehide", function () {',
    '      if (navigationState.controller) navigationState.controller.abort();',
    '      navigationState.controller = null;',
    '    });',
    '  }',
...
function buildRealtimeScript(options) {
  ...
  const realtimeStateKey = '__pocketpagesDatastarRealtime'

  return [
    '<script>',
    '(function () {',
    '  var stateKey = ' + scriptJson(realtimeStateKey) + ';',
    '  var endpoint = ' + scriptJson(endpoint) + ';',
    '  var topic = ' + scriptJson(topic) + ';',
    '  var clientIdSignal = ' + scriptJson(clientIdSignal) + ';',
    '  var previousState = window[stateKey];',
    '  if (previousState && previousState.endpoint === endpoint && previousState.topic === topic) {',
    '    previousState.connect();',
    '    return;',
    '  }',
    '  if (previousState) previousState.dispose();',
    '  var state = { endpoint: endpoint, topic: topic, source: null };',
    '  function disconnect() {',
    ...
    '    });',
    '  }',
    '  function handlePageHide() { disconnect(); }',
    '  function handlePageShow() { connect(); }',
